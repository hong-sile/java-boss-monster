...
- [x] 보스 몬스터의 HP가 0이 되거나 플레이어의 HP가 0이 될 때까지 게임을 진행한다.
- [x] 플레이어 이름은 5자 이하만 가능하다.
- [x] 플레이어의 초기 HP와 MP 합은 200이다.
- [x] 보스 몬스터 초기 HP는 100이상 300이하이다.
- [x] 플레이어는 매 턴마다 보스 몬스터를 공격할 수 있다.
    - [x] 물리 공격을 하면 보스 몬스터에게 10만큼의 데미지를 준다. (보스 몬스터의 HP가 10만큼 감소)
    - [x] 마법 공격을 하면 보스 몬스터에게 20만큼의 데미지를 준다. (보스 몬스터의 HP가 20만큼 감소)
    - [x] 마법 공격 시에는 MP 30이 소모된다.
    - [x] 물리 공격 시에는 MP 10을 회복한다. 단, 최대치 이상의 MP는 회복할 수 없다.
